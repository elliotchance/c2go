...
func transpileCallExpr(n *ast.CallExpr, p *program.Program) (
	...
	if p.IncludeHeaderIsExist("stdlib.h") && functionName == "calloc" &&
		len(n.Children()) == 3 {
		return callCalloc(n, p)
	}

	// function "qsort" from stdlib.h
	if p.IncludeHeaderIsExist("stdlib.h") && functionName == "qsort" &&
		len(n.Children()) == 5 {
		return callQsort(n, p)
	}
...
// callCalloc - call function "calloc" from #include <stdlib.h>
//
func callCalloc(n *ast.CallExpr, p *program.Program) (
	_ *goast.CallExpr, resultType string, preStmts []goast.Stmt, postStmts []goast.Stmt, err error) {
...
// callQsort - call function "qsort" from #include <stdlib.h>
// Example of clang AST:
...
//
func callQsort(n *ast.CallExpr, p *program.Program) (
	_ *goast.CallExpr, resultType string, preStmts []goast.Stmt, postStmts []goast.Stmt, err error) {
	defer func() {
		if err != nil {
