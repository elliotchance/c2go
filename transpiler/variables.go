...
func transpileMemberExpr(n *ast.MemberExpr, p *program.Program) (
	...
	if structType == nil {
		...
		err = fmt.Errorf("cannot determine type for LHS '"+lhsType+
			"', will use 'void *' for all fields. Is lvalue = %v", n.IsLvalue)
		p.AddMessage(p.GenerateWarningMessage(err, n))
	} else {
		if s, ok := structType.Fields[rhs].(string); ok {
			rhsType = s
		} else {
			err = fmt.Errorf("cannot determine type '%v' for RHS, will use"+
