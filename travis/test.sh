...
echo "Run: go test"
rm -f $OUTFILE
go list -f 'go test -v -tags integration -race -covermode atomic -coverprofile {{.Name}}.coverprofile -coverpkg $PKGS_DELIM {{.ImportPath}}' $PKGS > /tmp/go_test_command

filename="/tmp/go_test_command"
while read -r line
do
    name="$line"
